...
void arrInit (){
//  digitalWrite(ADXL_VCC2_PIN, HIGH); 
    if (!myAcc.init()) {
...
void sensor_loop(){
  ...
  if (currentMillis - loopMillis >= INTERVAL_TIME * mS_TO_S_FACTOR) {
    if(sleepCnt == 5){
      Serial.println("Sleep Now...");
      digitalWrite(ADXL_VCC_PIN, LOW);
      esp_sleep_enable_timer_wakeup(SLEEP_TIME * uS_TO_S_FACTOR);  // set wakeup timing
...
float getBatteryLevel(){
  ...
  Serial.print("Volts: ");
  Serial.println(BL.getBatteryVolts()/2);
  Serial.println("");
  return (float)BL.getBatteryVolts()/2;
}
