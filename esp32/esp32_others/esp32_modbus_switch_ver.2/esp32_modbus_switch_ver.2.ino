...
uint16_t switch_status[3] = {}; // 스위치 상태 0 꺼짐 201 켜짐
...
char valve_close_cmd[200] = "{\"value\":1001000,\"protocol\":1,\"repeat\":3}";
char valve_open_cmd[200] = "{\"value\":1001111,\"protocol\":1,\"repeat\":3}";

// pump control message
char pump_close_cmd[200] = "{\"value\":1002000,\"protocol\":1,\"repeat\":3}";
char pump_open_cmd[200] = "{\"value\":1002111,\"protocol\":1,\"repeat\":3}";

// fan control message
char fan_close_cmd[200] = "{\"value\":1003000,\"protocol\":1,\"repeat\":3}";
char fan_open_cmd[200] = "{\"value\":1003111,\"protocol\":1,\"repeat\":3}";
...
void loop_modbus(){
  ...
  yield();
  // 밸브 장치제어 커맨드 전송
  if(mb.Hreg(203) == 201 && switch_status[0] == 0){
    client.publish(device_command_topic,valve_open_cmd);
    switch_status[0] = 1;
    Serial.println("valve opened");
  }else if(mb.Hreg(203) == 0 && switch_status[0] == 1){
  ...
  // 유동 팬 장치제어 커맨드 전송
  if(mb.Hreg(211) == 201 && switch_status[2] == 0){
    client.publish(device_command_topic,fan_open_cmd);
    switch_status[2] = 1;
    Serial.println("fan opened");
  }else if(mb.Hreg(211) == 0 && switch_status[2] == 1){
    client.publish(device_command_topic,fan_close_cmd);
    switch_status[2] = 0;
