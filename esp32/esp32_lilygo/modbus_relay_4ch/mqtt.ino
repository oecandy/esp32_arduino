...
void mqttPublishHealthCheck(){
  snprintf(message, 200, "{\"ip\":\"%s\",\"rssi\":%ld,\"mac\":\"%s\",\"relayChannel\":%ld,\"serialNo\":\"%s\",\"heap\":%ld}", WiFi.localIP().toString().c_str(), WiFi.RSSI(), WiFi.macAddress().c_str(), RELAY_TYPE, SERIAL_NO, ESP.getFreeHeap());
